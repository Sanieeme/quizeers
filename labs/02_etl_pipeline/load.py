"""
Load step: write the cleaned data to a target store.

This targets a local SQLite database to keep the lab runnable anywhere, but
swapping the connection string for Postgres, Redshift, BigQuery, or Snowflake
is the only change needed to point this at a real warehouse.
"""
import sqlite3
import pandas as pd


def load_orders(clean: pd.DataFrame, rejected: pd.DataFrame, db_path: str) -> None:
    con = sqlite3.connect(db_path)
    try:
        clean.to_sql("orders", con, if_exists="replace", index=False)
        rejected.to_sql("orders_rejected", con, if_exists="replace", index=False)
        con.commit()
        print(f"[load] wrote {len(clean)} rows to 'orders' and {len(rejected)} to 'orders_rejected' in {db_path}")
    finally:
        con.close()


if __name__ == "__main__":
    from extract import extract_orders
    from transform import transform_orders

    raw = extract_orders("sample_data/orders_raw.csv")
    clean, rejected = transform_orders(raw)
    load_orders(clean, rejected, "output/warehouse.db")
