...
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///quizeer.db'
...
class Result(db.Model):
    ...
    quiz = db.relationship('Quiz', backref='results', lazy=True, cascade="all, delete-orphan", single_parent=True)
...
def register():
    if request.method == 'POST':
        ...
        show_password = 'showPassword' in request.form 
        ...
        if not any(char.isdigit() for char in password):
            flash('Password must include at least one numeric character.', 'danger')
            return redirect(url_for('register'))
...
def admin_delete_quiz(quiz_id):
    ...
    try:
        # The cascading delete will automatically handle deleting related results, questions, and answers
        db.session.delete(quiz)
        db.session.commit()

        flash('Quiz and related records deleted successfully!', 'success')
